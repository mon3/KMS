...
def calculate_potential(coordinates, R, e, N,):
	...
	rij = coordinates.reshape((N,1,3)) - coordinates.reshape((1,N,3)) 
	rij_scalar = np.sqrt((rij**2).sum(axis=2))
	indices = np.arange(int(N))
	rij_scalar[indices,indices] = np.inf
	potential_VDW = e*((R/rij_scalar)**12-2*(R/rij_scalar)**6) 
	total_VDW_potential = potential_VDW.sum() * 0.5
	f_part = e*((R/rij_scalar)**12-(R/rij_scalar)**6) 
	F_VDW = 12*f_part.reshape((N,N,1))*(rij/rij_scalar.reshape((N,N,1))**2)
	F_VDW = F_VDW.sum(axis = 1)
...
if __name__ == "__main__":
	...
	np.savetxt(coordinatesFile, particles[:,[0,1,2]]) 
	F, p, total_potential = calculate_potential(coordinates, R, e, N)
