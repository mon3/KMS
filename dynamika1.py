...
def calculate_potential(F,p, total_potential, momenta, coordinates):
...
def dynamics(momenta, coordinates):
	T_av = 0.0
	P_av = 0.0
	H_av = 0.0
	...
	for s in range(So+Sd):
		...
		calculate_potential(F, p, total_potential, momenta, coordinates)
		...
		energy_current = np.sum((momenta[:,0]*momenta[:,0]+momenta[:,1]*momenta[:,1]+momenta[:,2]*momenta[:,2])/(2.0*m))
		...
		if (fmod(s, Sout)==0.0):
			# t - czas
			t = s + tau
			f_handle = open(outFile, 'a')
			current_parameters = np.array([t,H,total_potential,energy_current,T,p])
			np.savetxt(f_handle, current_parameters.reshape((1,6)),delimiter='\t', fmt='%1.4e')
			f_handle.close()
